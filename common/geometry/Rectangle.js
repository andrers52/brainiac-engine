...
export default function Rectangle (centerPoint = new Vector(), size = new Vector()) {
...
Rectangle.prototype.setPointOfInterestAtPosition = function(pointOfInterest, position) {
...
Rectangle.prototype.allignToRectangle = function(
  otherRect,
  alligment) {
  Assert.assertIsValidString(
    alligment,
...
Rectangle.prototype.setPositionRelativeToRectangle = function(
  ...
  case 'randomBottom':
    ...
    break
  case 'center':
    Assert.assert(insideOutside == 'inside', 'Error: cannot position at center and outside at same time.')
  ...
  case 'randomLeft':
    ...
    this.move(vect(
      0,
      Random.randomFromIntervalInclusive(
        0,
        otherRect.size.y - this.size.y)))
    break
  case 'rightCenter':
    this.setPointOfInterestAtPosition(
      (insideOutside == 'inside')?
...
Rectangle.prototype.convert = function(fromBaseSize, toBaseSize) {
  return new Rectangle(this.center.convert(fromBaseSize, toBaseSize), this.size.convert(fromBaseSize, toBaseSize))
}
...
Rectangle.prototype.isMovingToInsideOfOtherRectangle = 
    function(movimentVector, otherRectangle) {
      let newPosition = this.center.clone().add(movimentVector)
      let currentPositionDistanceToOtherCenter = 
        this.center.distance(otherRectangle.center)
      let newPositionDistanceToOtherCenter = 
        newPosition.distance(otherRectangle.center)
        
      return newPositionDistanceToOtherCenter < currentPositionDistanceToOtherCenter
    }
...
Rectangle.prototype.positionPercentage = function(position) {
  if (!this.checkPointInside(position))
    return null
...
Rectangle.prototype.sizePercentageToPosition = function(percentage) {
  return (new Vector(
    ...
    this.size.y * percentage.y / 100)).subtract(this.size.clone().divideByScalar(2))
}

Rectangle.prototype.checkInside = function(rectangle) {
  return  (this.checkPointInside(rectangle.topLeft()) &&
...
Rectangle.prototype.checkHasCornerOutside = function(rectangle) {
  return  (this.checkPointOutside(rectangle.topLeft()) ||
...
function intersectionWithCornerInside(rectangle1, rectangle2) {
  return rectangle1.checkHasCornerInside(rectangle2) &&
...
function intersectionWithoutCornerInside(rectangle1, rectangle2) {
  ...
  return smallerX.bottomLeft().x >= greaterX.bottomLeft().x && smallerX.bottomLeft().x <= greaterX.bottomRight().x &&
            smallerX.bottomRight().x >= greaterX.bottomLeft().x && smallerX.bottomRight().x <= greaterX.bottomRight().x &&
            smallerY.bottomLeft().y >= greaterY.bottomLeft().y && smallerY.bottomLeft().y <= greaterY.topLeft().y &&
            smallerY.topLeft().y >= greaterY.bottomLeft().y && smallerY.topLeft().y <= greaterY.topLeft().y
...
Rectangle.prototype.checkIntersection = function(rectangle) {
  return (intersectionWithCornerInside(this, rectangle) ||
...
Rectangle.makeFromCorners = function(corner1, corner2) {
  let bottomLeft = new Vector(Math.min(corner1.x, corner2.x), Math.min(corner1.y, corner2.y))
  let topRight = new Vector(Math.max(corner1.x, corner2.x), Math.max(corner1.y, corner2.y))
  return new Rectangle(Vector.makeMean(bottomLeft, topRight), topRight.subtract(bottomLeft))
}

//create a new rectangle wich encloses all given rectangles
Rectangle.makeFromCollectionOfRectangles = function(rectangles) {
  Assert.assert(rectangles.length >= 1, 'Rectangle.makeFromCollectionOfRectanglesToBeEnclosed error: less than two rectangles given')

  if (rectangles.length === 1)
    return rectangles[0].clone()

  return rectangles.reduce(
    function(previousRectangle, currentRectangle) {
...
Rectangle.pickGreaterX = function(rectangle1, rectangle2) {
  return (rectangle1.getSize().x >= rectangle2.getSize().x) ? rectangle1 : rectangle2
}

Rectangle.pickGreaterY = function(rectangle1, rectangle2) {
  return (rectangle1.getSize().y >= rectangle2.getSize().y) ? rectangle1 : rectangle2
}
