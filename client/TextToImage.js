...
function getFontToFitTextOnRectangle(text, fontface, rectangle) {
  ...
  }while( context.measureText(text).width >= rectangle.size.x ||
...
TextToImage.drawText = function (
  ...
  let context = resourceStore.retrieveResourceObject(imageName).getContext('2d')
  //clear canvas
...
TextToImage.createRectangleFromTextAndFont = function(font, text) {
  ...
  Assert.assert(font, 'TextToImage#createRectangleFromTextAndFont Error: expecting font parameter to measure')
  Assert.assertIsLiteralString(font, 'TextToImage#createRectangleFromTextAndFont Error: font is not a string literal')
  let context = screen.getContext('2d')
  context.font = font
...
TextToImage.createImageFromText = function(
  ...
  Assert.assertIsLiteralString(text, 'BEClient.textToImagel Error: text is not a string literal')
  ...
  let grownRectangle = rect(rectangle.center.x, rectangle.center.y, rectangle.size.x, rectangle.size.y)
  grownRectangle.size.x *= 2
  grownRectangle.size.y *= 2

  let imageName = resourceStore.createNewImage(grownRectangle.size.x, grownRectangle.size.y, true)
