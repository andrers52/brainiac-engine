...
export default function Ship(context) {
  //points in canvas orientation (inverted y)
  let padding = 30
  let canvasTopLeft = vect(padding, padding)
  let inwardCenter = 
    vect(context.canvas.width/4+padding/4, context.canvas.height/2)
  let canvasBottomLeft = 
    vect(padding, context.canvas.height - padding)
  let canvasRightCenter = 
    vect(context.canvas.width - padding, context.canvas.height/2)
