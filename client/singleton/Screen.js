...
function Screen() {
  ...
  this.setBackgroundImageName = function (imageName) {
    backgroundImage = !!imageName && resourceStore.retrieveResourceObject(imageName)
  }
  ...
  function defineZoomOutFactor() {
    ...
    if ((screenSize.x >= minScreenDimension) && 
  ...
  function drawAgentImage(agent, canvasRectangle) {
    ...
    agent.opacity && (agent.opacity !== 0) && (context.globalAlpha = agent.opacity)
  ...
  function drawBackground() {
    ...
    let cameraInWorldTopLeftdistance =
      worldRectangle.topLeft()
        .vectorDistance(camera.rectangle.topLeft()).abs()

    //convert camera from world to backgroundImage
    let cameraRectangleInImage =
      camera.rectangle.convert(worldRectangle.size, imageRectangle.size)

    //sx,sy
    let cameraInImageTopLeftdistance =
      cameraInWorldTopLeftdistance
        .convert(worldRectangle.size, imageRectangle.size)
  ...
  this.drawAgent = function (agent) {
    ...
    let canvasRectangle =
      coordinatesConversion.rectangleWorldToCanvas(agent.rectangle)

    if (agent.imageName)
      drawAgentImage(agent, canvasRectangle)
  ...
  function drawWorldBorder() {
    ...
    let worldCanvasRectangle = 
      coordinatesConversion.rectangleWorldToCanvas(worldRectangle)
    let worldCanvasRectangleScreenToCameraFactor = worldCanvasRectangle
    worldCanvasRectangleScreenToCameraFactor.size.divideByScalar(screen.zoomOutFactor)
    ...
    context.rect(
      worldCanvasRectangleScreenToCameraFactor.center.x - worldCanvasRectangleScreenToCameraFactor.size.x / 2,
      worldCanvasRectangleScreenToCameraFactor.center.y - worldCanvasRectangleScreenToCameraFactor.size.y / 2,
  ...
  this.gamePresentationLoop = function () {
    ...
    onAfterDrawScreen &&
      onAfterDrawScreen(context)

    particlesContainer.animationStep()
    

    //presentationLoopId = window.requestAnimationFrame(screen.gamePresentationLoop)
    presentationLoopId = window.setTimeout(screen.gamePresentationLoop, BEClientDefinitions.ANIMATION_INTERVAL)
  }
