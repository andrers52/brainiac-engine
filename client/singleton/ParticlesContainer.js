...
import Time from '../../../arslib/time/time.js'
...
function ParticlesContainer () {
  ...
  this.createParticle = function ({
    ...
    //set defaults
    colorRed =
      (typeof colorRed === 'number')?
        colorRed : 
        255
    colorGreen =
      (typeof colorGreen === 'number')?
        colorGreen : 
        255
    colorBlue =
      (typeof colorBlue === 'number')?
        colorBlue : 
        255
    ...
    occurrenceProbability = 
      (typeof occurrenceProbability === 'number')?
        occurrenceProbability : 
        1

    
    if(!Random.occurrenceProbability(
      occurrenceProbability)) return
    ...
    for(let particleCount=0;particleCount<occurrenceProbability;particleCount++) {
      let particle = {
        positionX, positionY,
        speedX: adjustedParticleSpeed.x, speedY: adjustedParticleSpeed.y,
        screenParticle,
  ...
  this.createParticleInSpray = function (
    ...
    timeToLive += Random.randomFromInterval(-timeToLiveVariation, timeToLiveVariation)
  ...
  function runParticles() {
    for (let id in particles) {
      ...
      if (particle.timeToLive <= timeAlive)
        ...
        particle.opacity = remainingTimeToLive / particle.timeToLive
    }
  }
  ...
  function drawParticleComponents(particle, context) {
    ...
    for (let colorComponent in colorToParticleImage) {
      ...
      context.globalAlpha =
        fromColorToOpacity(particle.color[colorComponent]) *
        particle.opacity
      ...
      context.drawImage(
        ...
        particle.radius * 2/screen.zoomOutFactor, particle.radius * 2/screen.zoomOutFactor)
    }
  }
  ...
  this.createCircularEmitter = function (
    ...
    if(ownerAgentIdInput) {
      let ownedEmitter = emitters.find(emitter => emitter.ownerAgentId === ownerAgentIdInput)
      if(ownedEmitter) {
    ...
    emitter.run = 
      EFunction.limitCallingRateWithDiscard(runWithoutRateLimit, particleReleaseLatency)
    ...
    emitters =
      emitters.filter(
        emitter => {
          ...
          return Time.currentTime() - emitter.creationTime <
            emitter.timeToLive
        })
  }
