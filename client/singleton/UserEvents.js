...
function UserEvents () {
  ...
  this.mouseWorldPosition = function () {
    // try {
    return coordinatesConversion.canvasToWorld(mouseCanvasPosition)
    // } catch (e) {
  ...
  function getMouseCanvasPosition(event) {
    ...
    let resultVector = new Vector(event.clientX - rect.left, event.clientY - rect.top)
  ...
  function onResizeCanvas(event) {
    ...
    propagate('onResizeCanvas', vect(screen.getCanvas().width, screen.getCanvas().height))
  }
  ...
  function onMouseDown(event) {
    ...
    propagate('onMouseDown', userEvents.mouseWorldPosition())
    event.stopPropagation()
  }

  function onMouseUp(event) {
    mouseCanvasPosition = getMouseCanvasPosition(event)
    propagate('onMouseUp', userEvents.mouseWorldPosition())
  }
  ...
  this.enableEvent = function (handler) {
    ...
    eventsMapping[handler].forEach(event => eval(event + ' = ' + handler + ';'))
  }
  ...
  function propagateMouseMoveOnInterval() {
    if (mousePositionChanged) {
      propagate('onMouseMove', userEvents.mouseWorldPosition())
  ...
  this.start = function (mouseMovePropagationLatency, propagateInput) {
