...
import {ImageUtil} from '../../../arslib/util/image-util.js'
...
function ResourceStore () {
  ...
  function getMimeType(identifier) {
    ...
    Assert.assert(resourceTypesToMime.hasOwnProperty(suffix), `resourceStore#getMimeType suffix not found ${suffix}`)
  ...
  this.createEffectsFromDescriptor = function(descriptorObj) {
    descriptorObj.images.forEach( function(newImage) {
      ...
      for (const newEffect of newImage.effectsToApply) {
        ...
        Effect(imageName, size, opacity, fillColor, strokeColor, newImageName, effectName, parameters, combineOption)
  ...
  this.callWhenReady = function (functionToCall) {
    if (resources_loading === 0) {
      if (effectsDescriptor) this.createEffectsFromDescriptor(JSON.parse(effectsDescriptor))
      functionToCall()
      return
    }
  ...
  function decrease_resources_counter() {
    Assert.assert(resources_loading > 0, 'Error in resource loading counting in Resource Store...')
    resources_loading--
  }
  ...
  function storeAudioData(identifier, audioDataSrc) {
    ...
    audio.src = (typeof (audioDataSrc) === 'string') ?
  ...
  function loadImage(identifier) {
    Assert.assert(identifier, 'resourceStore#loadImage error: image not defined')
    try {
      ...
      throw ('resourceStore#loadImage error trying to load image ' + identifier + '\n Details: \n ' + err)
    }
  }


  function loadAudio(identifier) {
    Assert.assert(identifier, 'resourceStore#loadAudio error: image not defined')
    ...
    try {
      ...
      request.onloadend = () => {
        ...
        if(EArray.last(identifier.split('/')) ==='effects_descriptor.json') 
          ...
          resourceStore.resources[identifier] = JSON.parse(jsonDescriptor)
      ...
      throw ('resourceStore#loadJSON error trying to read JSON file ' + identifier + '\n Details: \n ' + err)
    }
  }
  ...
  this.createNewImageName = function (permanent = false) {
    ...
    do {
      newName = `${permanent? '' : 'temp_'}image_${(Random.randomInt(100000000)).toString()}.jpg`
  ...
  this.removeResourceIfTemporary = function (resourceName) {
    if (resourceName.includes('temp_'))
  ...
  this.cloneImage = function (imageName) {
    ...
    let newCanvas = ImageUtil.createCanvas(originalImage.width, originalImage.height)
  ...
  this.retrieveResourceObject = function (identifier) {
    ...
    if (!this.checkResourceObjectExists(identifier)) {
      ...
      console.log('Resource Store error: trying to retrieve unknown resource: ' + identifier)
  ...
  this.hasResource = function (resourceToCheck) {
    return this.resources.hasOwnProperty(resourceToCheck) && this.resources[resourceToCheck]
  }

  function retrieveAllResourceNamesBySuffix(suffix) {
    let mimeType = getMimeType(suffix)
    let result = []
    Object.keys(resourceStore.resources).
      forEach(
        identifier => {
          if (getMimeType(identifier) === mimeType)
