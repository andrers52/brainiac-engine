...
import Sound from '../../../arslib/util/sound.js'
import userEvents from './UserEvents.js'
import coordinatesConversion from './CoordinatesConversion.js'

// changing defaults
import BrowserUtil from '../../../arslib/util/browser-util.js'
...
BEClient.setVisibleAgents = function (visibleAgentsInput) {
  ...
  if(BEClient.config.userAlwaysAtCenterOfCamera) {
    const userAgent =
      BEClient.visibleAgents.find(agent =>
        !!agent.name && agent.name === BEClient.userName)
...
function setCamera(cameraToSet) {
  Object.assign(camera, cameraToSet) // keep reference
  EObject.extend(camera.rectangle, Rectangle.prototype)
  EObject.extend(camera.rectangle.center, Vector.prototype)
  EObject.extend(camera.rectangle.size, Vector.prototype)  
}
...
function removeAllAudioEvents() {
  resourceStore.retrieveAllAudioNames().map(audioName => Sound.clearAllEvents(audioName))
}
...
function propagateUserEvent(event, arg) {
  ...
  //propagate to server
  BEClient.socket.emit(
    'userEvent',
    eventAndArg)
  if(event.includes('Mouse')) return
  //propagate to client
  window.dispatchEvent(new CustomEvent(event, {
    detail: arg, bubbles: false, composed: true
  }))
}
...
function _startConnection() {
  ...
  if(BEClient.config.localApp) {
    ...
    BEClient.socket = io(BECommonDefinitions.WEB_SOCKET_ADDRESS)
  ...
  BEClient.socket.on('messageToGameClient', function (messageAndContentObj) {
    if(!BEClient.app[messageAndContentObj.message])
      console.log('Server message not understood: ' + messageAndContentObj.message)
  ...
  BEClient.socket.on('BEClient.localization.setCurrentLanguage', function (language) {
...
BEClient.start = function (app) {
  ...
  let executeAfterLoadCommonResources = () => {
    ...
    BEClient.config = resourceStore.retrieveResourceObject(BECommonDefinitions.CONFIG_JSON)
    _startConnection()
...
BEClient.connectToGameServer = function () {
  ...
  let executeAfterLoadGameResources = async () => {
    ...
    BEClient.app.onAfterDrawAgent &&
      (BEClient.onAfterDrawAgent = BEClient.app.onAfterDrawAgent.bind(BEClient.app))
    BEClient.app.onBeforeDrawAgent &&
      (BEClient.onBeforeDrawAgent = BEClient.app.onBeforeDrawAgent.bind(BEClient.app))
    BEClient.app.onAfterDrawScreen &&
      (BEClient.onAfterDrawScreen = BEClient.app.onAfterDrawScreen.bind(BEClient.app))
    ...
    BEClient.socket.on(
      'BEServer.clientStartReady', function (userIdAndBackgroundImageName) {
        let userId = userIdAndBackgroundImageName.userAgentId
        BEClient.setBackgroundImageName(userIdAndBackgroundImageName.backgroundImagename)
        //server is ready
        //removeLoadingImage();
        ...
        userEvents.start(
          ...
          propagateUserEvent)
        coordinatesConversion.start(camera)
  ...
  Assert.assertIsFunction(BEClient.app.getMediaAssets, 'You need to define \'getMediaAssets\' on the game to be able to load your media assets.')
  let gameMediaAssets = BEClient.app.getMediaAssets()
  gameMediaAssets.forEach(
    asset => 
      resourceStore.addResource(asset)
  )
...
function loadCommonResources() {
  resourceStore.createEffectsFromDescriptor(BECommonDefinitions.COMMON_EFFECTS_DESCRIPTION)
  BECommonDefinitions.COMMON_RESOURCES.forEach(resource => resourceStore.addResource(resource))
}

export {BEClient as default}
