...
import Platform from '../../../arslib/util/platform.js'
...
function BEServerConstructor() {
  ...
  async function _readConfig() {
    if(!Platform.isNode()) {
      ...
      const resultStr = await readFileAsync(BECommonDefinitions.CONFIG_JSON, {encoding: 'utf8'})
  ...
  function _sendClientVisibleAgents() {
    ...
    setTimeout(_sendClientVisibleAgents, AgentDefinitions.AGENTS_CLIENT_REFRESH_INTERVAL)
  }

  this.start = function() {
    environment.start(BECommonDefinitions.WORLD_WIDTH, BECommonDefinitions.WORLD_HEIGHT)
    ...
    _readConfig()
      .then(config => {
        ...
        if(BECommonDefinitions.config.buildType === 'deploy') Assert.disableAllVerifications = true    
