...
function ConnectorConstructor() {
  ...
  this.getUserById = function (id) { return idToUsers[id] }
  ...
  this.playSoundInClient = function (soundName) {
    ...
    for (var id in idToUsers) {
      ...
      user.socket.emit('Sound.playSound',
        soundName)
    }
  ...
  this.playProceduralSoundInClient = function (soundDescObj, generatingEventPosition) {
  ...
  this.playSoundInClientLoop = function (soundName) {
    ...
    for (var id in idToUsers) {
      let user = idToUsers[id]
      user.socket.emit('Sound.playSoundLoop',
        soundName)
    }
  }
  ...
  this.setVisibleAgents = function () {
    for (var id in idToUsers) {
      ...
      let nearbyAgents = environment.getNearbyAgentsByRectangle(user.camera.rectangle)

      let userVisibleAgents =
        nearbyAgents.filter(
  ...
  this.messageToSingleGameClient = function (userId, message, contentObject) {
    ...
    try {
      idToUsers[userId].socket.emit(
        'messageToGameClient',
        { 'message': message, 'contentObject': contentObject })
    }
    catch (e) {
      console.log(`data encoding error. Message is:${message},  contentObject is: ${JSON.stringify(contentObject)}`)
    }
  }
  ...
  this.start = function (localApp) {
    if(!localApp) {
      ...
      server.listen(BECommonDefinitions.WEB_PORT, function () {
        console.log('Web server listening at port %d', BECommonDefinitions.WEB_PORT)
      })
    ...
    io.on('connection', function (socketInput) {
      ...
      socket.on('BEServer.clientStart', function (startAppArgs) {
        ...
        if(BEServer.config.cameraFollowUser) {
          ...
          user.camera.start(cameraSize, vect(0,0))  

        //tell the client we are ready
        socket.emit(
          'BEServer.clientStartReady',
          {userAgentId: user.agent? user.agent.id : 0, backgroundImagename: BEServer.getBackgroundImageName()})
