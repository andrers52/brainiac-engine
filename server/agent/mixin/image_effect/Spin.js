...
export default function Spin (maxTimeinMilliseconds) {
  let DEFAULT_MAX_TIME = 1000
  let NUM_SPINS = 360
  let spinTimeInMilliseconds = (maxTimeinMilliseconds || DEFAULT_MAX_TIME)/NUM_SPINS
  let self = this
  let spinIntervalId
  ...
  this.startSpinning = function() {
    !spinIntervalId && (spinIntervalId = setInterval(spin, spinTimeInMilliseconds))
    maxTimeinMilliseconds && setTimeout(clearSpin, maxTimeinMilliseconds)
  }

  this.die =
    EFunction.sequence(clearSpin, this.die, this)
}
