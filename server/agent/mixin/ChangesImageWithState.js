...
//  ]
//}
export default function ChangesImageWithState (configurationObj) {
  ...
  function start(configurationObj) {
    try {
      ...
      throw 'ChangesImageWithState initialization error: resources descriptor reading: ' + exception
    }
    Assert.assert(states.includes(defaultState), 'ChangesImageWithState error: resources integrity check failed')
  ...
  function updateImageAndAudio(state) {
    ...
    if((self.getSize().x === 0) && (self.getSize().x === 0)) { //agent may have size.x === size.y === 0 because it was initialized without a rectangle
      self.rectangle = new Rectangle(self.rectangle.getPosition() || null,
        new Vector(10, 10))
    }
  ...
  this.addActionToExecuteAtStateChange = function(state, action) {
    Assert.assert(isValidState(state),'ChangesImageWithState#addActionToExecuteAtStateChange error -> invalid state')
    Assert.assertIsFunction(action, 'ChangesImageWithState#addActionToExecuteAtStateChange error -> action expected to be a function')
