...
export default function SensingAgent (detector = () => false, sensingDistance = 100, delay = 250) {
  Assert.assert(
    this.onSensingAgents || this.onSensingForwardAgents || 
    this.onSensingUserAgent || this.onSensingMostForwardAgent,
  ...
  let sensingRect =
    rect(0, 0, sensingDistance * 2, sensingDistance * 2)
  let self = this

  function calculateRelativeDetectionVector(detectedAgent) {
    let absoluteDetectionVector =
      detectedAgent.getPosition()
        .clone().subtract(self.getPosition())

    let absoluteDetectionVectorAngle =
      absoluteDetectionVector.getAngle()
    let absoluteDetectionVectorSize =
      absoluteDetectionVector.size()

    let relativeDetectionVectorAngle =
      absoluteDetectionVectorAngle -
      self.orientation

    return Vector.makeFromAngleAndSize(
      relativeDetectionVectorAngle, absoluteDetectionVectorSize)
  }
  ...
  function getSensingAgents() {
    ...
    let vectorsAndAgents = detectedNearbyAgents.map(
      agent =>
        ({
    ...
    Assert.assertIsArray(vectorsAndAgents, 'SensingAgent#getSensingAgents should return an array of objects')
    Assert.assert(vectorsAndAgents[0].vector && vectorsAndAgents[0].agent, 'SensingAgent#getSensingAgents object seems mal-formed')
  ...
  function findMostForwardAgent(forwardVectorsAndAgents) {
    ...
    let mostForwardVectorAndAgentAngle = Math.abs(mostForwardVectorAndAgent.vector.getAngle())
  ...
  function getUserAgentAndVector (vectorsAndAgents) {
    let vectorAndAgent =
    vectorsAndAgents.filter(vectorAndAgent => vectorAndAgent.agent.isUserAgent())
  ...
  function testSensingAtInterval() {
    self.isAlive &&
      setTimeout(testSensingAtInterval, delay)
    ...
    if (self.onSensingForwardAgents || self.onSensingMostForwardAgent) {
      let forwardVectorsAndAgents =
        vectorsAndAgents.filter(vectorAndAgent => isForward(vectorAndAgent.vector))
      if (forwardVectorsAndAgents.length > 0) {
        self.onSensingForwardAgents && self.onSensingForwardAgents(forwardVectorsAndAgents)
        if (self.onSensingMostForwardAgent) {
          let mostForwardVectorAndAgent = findMostForwardAgent(forwardVectorsAndAgents)
