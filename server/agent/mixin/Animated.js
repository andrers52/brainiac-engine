...
export default function Animated (configurationObj, soundProvider) {
  ...
  function start(configurationObj) {
    try {
      ...
      configurationObj.animationStates.forEach(function(animationState) {
        ...
        framesPerState[animationState.stateName] = animationState.animationFrames
        timePerState[animationState.stateName] = animationState.timeinMilis || ANIMATION_DEFAULT_DELAY
    ...
    Assert.assert(states.includes(defaultState), 'Animation resources integrity check failed')
  ...
  function changeFrame() {
    ...
    if (!auto_reverse) {
      if (isLastFrame())
        currentStateFrame = 0 //restart on last frame
      else
        currentStateFrame++
    ...
    if((isFirstFrame() && current_increment === -1) ||
  ...
  this.addActionToExecuteAtAnimationEnd = function(state, action) {
    Assert.assert(isValidState(state),'Animated#addActionToAnimationEnd error -> given state is not valid')
    Assert.assertIsFunction(action, 'Animated#addActionToAnimationEnd error -> action expected to be a function')
  ...
  this.die =
    EFunction.sequence(this.die, animatedDie, this)
