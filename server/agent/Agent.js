...
function agentInitialize(imageName, inputRectangle, isSolid) {
  ...
  if (this.isSolid) {
    let overlappingAgent = environment.otherAgentOverlappingWithProposedRectangle(this, this.rectangle)
...
let createAgent = function (imageName, width = 100, height = 100, isSolid = true, xPos = 0, yPos = 0) {
  return createAgentWithRectangle(imageName, rect(xPos, yPos, width, height), isSolid)
}

//use this to create a new agent with specific rectangle
let createAgentWithRectangle = function (imageName, inputRectangle, isSolid = true,) {
...
let proto = {
  ...
  forceMove(distance) {
    Assert.assert(typeof distance.x === 'number' && typeof distance.y === 'number')
  ...
  checkMove(distance) {
    ...
    let overlappingAgent = environment.otherAgentOverlappingWithProposedRectangle(this, testRectangle)
    ...
    if (this.isSolid && overlappingAgent.isSolid) {
      ...
      if(this.rectangle.isMovingToInsideOfOtherRectangle(
        distance,
        overlappingAgent.rectangle)) return null
  ...
  moveTowardsPosition(positionToGetClose, scalarDistanceToMove) {
    let vectorDistanceFromTarget = this.getPosition().vectorDistance(positionToGetClose)
    let vectorDistanceFromTargetToMove = vectorDistanceFromTarget.clone().adjustToSize(scalarDistanceToMove)
  ...
  setPositionRelativeToAgent(otherAgent,
    relativePosition,
  ...
  moveRelativeToAgent(otherAgent, positionRelativeToOtherAgent, padding, force) {
