...
function Environment() {
  ...
  this.otherAgentOverlappingWithProposedRectangle = function (agent, proposedRectangleInput) {
    ...
    for(let index = 0; index<nearbyAgents.length; index++) {
      ...
      let dist =
        proposedRectangle.center.distance(nearbyAgent.rectangle.center)
      //let agentRadius = proposedRectangle.meanSize()/2;
...
*/
  ...
  this.start = function (WORLD_WIDTH, WORLD_HEIGHT) {
    worldRectangle = 
      new Rectangle(new Vector(), new Vector(WORLD_WIDTH, WORLD_HEIGHT))
