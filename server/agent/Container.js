...
import Vector from '../../../../arslib/geometry/Vector.js'
import Rectangle from '../../../../arslib/geometry/Rectangle.js'
...
export default function createContainer (agentsToContain, orientation, padding, rectangle) {

  let container = createAgent(null, 100, 100, false)
  orientation && Assert.assertIsValidString(orientation, [ 'vertical', 'horizontal'], 'Invalid container orientation given:' + orientation)
  ...
  function adjustAgentsSizes() {
    ...
    let totalHorizontalSize = container.rectangle.size.x - ((container.orientation === 'horizontal')? totalPadding : 0)
    let agentHorizontalSize = totalHorizontalSize / ((container.orientation === 'horizontal')? containedCollection.length : 1)
    let totalVerticalSize =  container.rectangle.size.y - ((container.orientation === 'vertical')? totalPadding : 0)
    let agentVerticalSize = totalVerticalSize / ((container.orientation === 'vertical')? containedCollection.length : 1)
    containedCollection.forEach(function(agent) {agent.setSize(new Vector(agentHorizontalSize,agentVerticalSize))})
  }

  function placeRemainingAgentsWithFirstAsAnchor() {
    let agentOrientationEquivalence = {'vertical': 'below', 'horizontal': 'right'}
  ...
  function placeAllAgents() {
    ...
    containedCollection.first().setPosition(
      new Vector(
        xPos,
        yPos), true)
  ...
  agentsToContain && agentsToContain.forEach(agent => container.addAgent(agent))
  ...
  container.move = function(distance, force) {
    if(!force) {
      if (!container.checkMove(distance))
        return false
      for(let i=0;i<containedCollection.length;i++) {
        if(!containedCollection[i].checkMove(distance))
          return false
      }
    }
    ...
    containedCollection.forEach(agent => originalMove.call(agent,distance,true))
    return true
  }
  ...
  container.die =
    EFunction.sequence(killAll, originalDie, container)
